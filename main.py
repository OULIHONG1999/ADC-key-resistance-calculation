...
# E48 系列（公差 ±2%）
E48 = [1.00, 1.05, 1.10, 1.15, 1.21, 1.27, 1.33, 1.40, 1.47, 1.54, 1.62, 1.69, 1.78, 1.87, 1.96, 2.05, 2.15, 2.26, 2.37,
       2.49, 2.61, 2.74, 2.87, 3.01, 3.16, 3.32, 3.48, 3.65, 3.83, 4.02, 4.22, 4.42, 4.64, 4.87, 5.11, 5.36, 5.62, 5.90,
...
E192 = [1.00, 1.01, 1.02, 1.04, 1.05, 1.06, 1.07, 1.09, 1.10, 1.11, 1.13, 1.14, 1.15, 1.17, 1.18, 1.20, 1.21, 1.23,
...
def find_closest_resistor(target, series=E, multiplier_range=(1, 1000000)):
    ...
    """
    all_resistors = []
    for multiplier in [10 ** i for i in
                       range(len(str(int(min(multiplier_range)))), len(str(int(max(multiplier_range)))))]:
...
def get_variable_name_of_E():
    """
    获取当前 E 变量对应的变量名。

    返回:
    str: E 对应的变量名，如果未找到则返回空字符串。
    """
    resistor_series_map = {
        "E6": E6,
        "E12": E12,
    ...
    for name, series in resistor_series_map.items():
        if series is E:
            return name
    return ""
...
if __name__ == "__main__":
    # 不同公比列表，调整范围为 0~1
    r_values = np.linspace(0.6, 0.8, 5)
    ...
    for r in r_values:
        # 打印E的变量名
        e_name = get_variable_name_of_E()
        if e_name:
            print(f"E 是: {e_name}")
        result = calculate_adc_values(r=r)
        if result:
